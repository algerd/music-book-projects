...
public class InstrumentsPaneController implements Initializable, Loadable {
        ...
        numberOfMusiciansColumn.setCellFactory(col -> {
            TableCell<Instrument, Instrument> cell = new TableCell<Instrument, Instrument>() {
                ...
                public void updateItem(Instrument item, boolean empty) {
                    ...
                    this.setText(null);
                    if (!empty) {                        
                        this.setText("" + Main.getInstance().getDbLoader().getMusicianInstrumentTable().countMusiciansByInstrument(item));                   
                    }
                }
            };
			return cell;
        ...
        averageRatingColumn.setCellFactory(col -> {
            TableCell<Instrument, Instrument> cell = new TableCell<Instrument, Instrument>() {
                ...
                public void updateItem(Instrument item, boolean empty) {
                    ...
                    this.setText(null);
                    if (!empty) {                        
                        List<MusicianInstrument> musicianInstruments = Main.getInstance().getDbLoader().getMusicianInstrumentTable().selectJoinByInstrument(item);
                        int averageRating = 0;
    ...
    private void setTableValue() {
        instruments = Main.getInstance().getDbLoader().getInstrumentTable().select();    
    ...
    private void initTableListeners() {      
        ...
        instrumentsTable.getSelectionModel().selectedItemProperty().addListener(
            (observable, oldValue, newValue) -> {             
                selectedItem = instrumentsTable.getSelectionModel().getSelectedItem();
            }
        );  
