...
public class SongDialogController implements Initializable, DialogController {
    ...
    public void initialize(URL url, ResourceBundle rb) {
        ...
        artistField.getItems().addAll(Main.getInstance().getDbLoader().getArtistTable().select());
    ...
    private void changedArtistChoiceBox(ObservableValue<? extends Object> observable, Artist oldValue, Artist newValue) {          
        ...
        albumField.getItems().addAll(Main.getInstance().getDbLoader().getAlbumTable().selectByArtist(artist));
    ...
    private void initGenreChoiceCheckBox() {
        ...
        if (edit) {        
            Main.getInstance().getDbLoader().getSongGenreTable().selectJoinBySong(song).stream().forEach(artistGenre -> {
    ...
    private void handleOkButton() {
        if (isInputValid()) {
            ...
            */
            if (!edit) {
                Main.getInstance().getDbLoader().getSongTable().save(song);   
            } else {           
                // Cначала удалить все жанры из бд для песни, а потом добавить
                Main.getInstance().getDbLoader().getSongGenreTable().deleteBySong(song);
            } 
            ...
            for (Genre genre : includedChoiceCheckBoxController.getItemMap().keySet()) {
                ...
                if (value.getValue()) {
                    ...
                    Main.getInstance().getDbLoader().getSongGenreTable().save(songGenre);
                }
            }
    ...
    private boolean isInputValid() {
        ...
        if (!edit && Main.getInstance().getDbLoader().getSongTable().containsSong(nameField.getText(), albumField.getValue())) {
