...
public class AlbumDialogController implements Initializable, DialogController {
    ...
    private void initGenreChoiceCheckBox() {
        ...
        if (edit) {        
            Main.getInstance().getDbLoader().getAlbumGenreTable().selectJoinByAlbum(album).stream().forEach(artistGenre -> {
    ...
    private void handleOkButton() {
        if (isInputValid()) {
            ...
            */
            if (!edit) {
                Main.getInstance().getDbLoader().getAlbumTable().save(album);   
            } else {           
                // Cначала удалить все жанры из бд для артиста, а потом добавить
                Main.getInstance().getDbLoader().getAlbumGenreTable().deleteByAlbum(album);
            } 
            ...
            for (Genre genre : includedChoiceCheckBoxController.getItemMap().keySet()) {
                ...
                if (value.getValue()) {
                    ...
                    Main.getInstance().getDbLoader().getAlbumGenreTable().save(albumGenre);
                }
            }
    ...
    private boolean isInputValid() {
        ...
        if (!edit && Main.getInstance().getDbLoader().getAlbumTable().containsAlbum(nameField.getText(), artistField.getValue())) {
