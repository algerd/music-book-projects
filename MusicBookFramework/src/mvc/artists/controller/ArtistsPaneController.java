...
public class ArtistsPaneController implements Initializable, Loadable {
    ...
    private void initArtistsTable() { 
        ...
        artistsTable.getSelectionModel().selectedItemProperty().addListener(
            (observable, oldValue, newValue) -> {             
                selectedItem = artistsTable.getSelectionModel().getSelectedItem();
            }
        );
        Main.getInstance().getDbLoader().getArtistTable().addedProperty().addListener(this::added);          
        Main.getInstance().getDbLoader().getArtistTable().deletedProperty().addListener(this::deleted);           
        Main.getInstance().getDbLoader().getArtistTable().updatedProperty().addListener(this::updated);
    ...
    private void filter() {
        ...
        for (Artist artist : artists) {          
            ...
            if (genre.getId() != -1) {              
                List<ArtistGenre> artistGenres = Main.getInstance().getDbLoader().getArtistGenreTable().selectJoinByArtist(artist);
    ...
    public void setTableValue() {
        artists = Main.getInstance().getDbLoader().getArtistTable().select();
