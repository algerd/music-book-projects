...
public class SongsPaneController implements Initializable, Loadable  {
    ...
    private void setTableValue() {
        songs = Main.getInstance().getDbLoader().getSongTable().selectJoin();    
    ...
    private void initTableListeners() {
        ...
        songsTable.getSelectionModel().selectedItemProperty().addListener(
            (observable, oldValue, newValue) -> {             
                selectedItem = songsTable.getSelectionModel().getSelectedItem();
            }
        );      
        ...
        Main.getInstance().getDbLoader().getAlbumTable().addedProperty().addListener(this::changed);          
        ...
        Main.getInstance().getDbLoader().getSongTable().addedProperty().addListener(this::changed);          
    ...
    private void filter() {
        ...
        for (Song song : songs) {
            ...
            if (genre.getId() != -1) {              
                List<SongGenre> songGenres = Main.getInstance().getDbLoader().getSongGenreTable().selectJoinBySong(song);
    ...
    private void onMouseClickTable(MouseEvent mouseEvent) { 
        ...
        if (mouseEvent.getButton() == MouseButton.PRIMARY) {
            ...
            if (selectedItem != null) {
                Song song = Main.getInstance().getDbLoader().getSongTable().select(selectedItem.getId());
