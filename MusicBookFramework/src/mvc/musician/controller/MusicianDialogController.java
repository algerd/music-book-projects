...
public class MusicianDialogController implements Initializable, DialogController {
    ...
    public void initialize(URL url, ResourceBundle rb) {
        ...
        includedGenreChoiceCheckBoxController.setMainPane(dialogPane);
        ...
        includedInstrumentChoiceCheckBoxController.setMainPane(dialogPane);
    ...
    private void initGenreChoiceCheckBox() {
        ...
        if (edit) {        
            Main.getInstance().getDbLoader().getMusicianGenreTable().selectJoinByMusician(musician).stream().forEach(musicianGenre -> {
    ...
    private void initInstrumentChoiceCheckBox() {
        ...
        if (edit) {        
            Main.getInstance().getDbLoader().getMusicianInstrumentTable().selectJoinByMusician(musician).stream().forEach(musicianInstrument -> {
                musicianInstruments.add(musicianInstrument.getInstrument());
            });
        }
        Map<Instrument, ObservableValue<Boolean>> map = new HashMap<>();
        Main.getInstance().getDbLoader().getInstrumentTable().select().stream().forEach(instrument -> {                     
    ...
    private void handleOkButton() {      
        if (isInputValid()) {
            ...
            */
            if (!edit) {
                Main.getInstance().getDbLoader().getMusicianTable().save(musician);   
            } else {           
                // Cначала удалить все жанры из бд для музыканта, а потом добавить
                Main.getInstance().getDbLoader().getMusicianGenreTable().deleteByMusician(musician);
                // Cначала удалить все инструменты из бд для музыканта, а потом добавить
                Main.getInstance().getDbLoader().getMusicianInstrumentTable().deleteByMusician(musician);
            } 
            ...
            for (Genre genre : includedGenreChoiceCheckBoxController.getItemMap().keySet()) {
                ...
                if (value.getValue()) {
                    ...
                    Main.getInstance().getDbLoader().getMusicianGenreTable().save(musicianGenre);
                }
            }  
            ...
            for (Instrument instrument : includedInstrumentChoiceCheckBoxController.getItemMap().keySet()) {
                ...
                if (value.getValue()) {
                    ...
                    Main.getInstance().getDbLoader().getMusicianInstrumentTable().save(musicianInstrument);
                }
            }
    ...
    private boolean isInputValid() {
        ...
        if (!edit && !Main.getInstance().getDbLoader().getMusicianTable().isUniqueColumnValue("name", nameTextField.getText())) {
    ...
    public void setEntity(Entity entity) {
        if (entity != null && entity instanceof Musician) {
